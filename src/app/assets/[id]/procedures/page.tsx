    ...
    <div className="app-shell">
      ...
      <main className="page-main">
        ...
        {emergencies.length > 0 && <section id="emergency" className="card" style={{ border: "3px solid currentColor" }} aria-labelledby="emergency-heading">
          ...
          <div className="stack">{emergencies.map((procedure) => <details key={procedure.id}><summary><strong>{procedure.title}</strong></summary>{procedure.notes && <p>{procedure.notes}</p>}<ol style={{ fontSize: "1.05rem", lineHeight: 1.6 }}>{procedure.steps.map((step) => <li key={step.id ?? step.position}>{step.instruction}{step.note ? ` — ${step.note}` : ""}</li>)}</ol>{procedure.sourceDocumentTitle && <p className="muted"><strong>Source:</strong> {procedure.sourceDocumentTitle}{procedure.sourcePage ? ` · page ${procedure.sourcePage}` : ""}</p>}<ProcedureEditor assetId={asset.id} procedure={procedure} /></details>)}</div>
        </section>}
        ...
        <section className="card"><h2>Routine procedures</h2>{routines.length === 0 ? <p className="muted">No verified routine procedures yet.</p> : routines.map((procedure) => <details key={procedure.id}><summary><strong>{procedure.title}</strong> <span className="muted">· {procedure.steps.length} steps</span></summary>{procedure.notes && <p>{procedure.notes}</p>}<ol>{procedure.steps.map((step) => <li key={step.id ?? step.position}>{step.instruction}{step.note ? ` — ${step.note}` : ""}</li>)}</ol>{procedure.sourceDocumentTitle && <p className="muted"><strong>Source:</strong> {procedure.sourceDocumentTitle}{procedure.sourcePage ? ` · page ${procedure.sourcePage}` : ""}</p>}<ProcedureEditor assetId={asset.id} procedure={procedure} /></details>)}</section>
