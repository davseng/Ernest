...
function ProposalDetails({ proposal }: { proposal: ErnestWriteProposal }) {
  ...
  if (proposal.kind === "component_fact") return <dl className="write-proposal-details"><div><dt>Component</dt><dd>{proposal.componentFact.componentName}</dd></div><div><dt>Field</dt><dd>{proposal.componentFact.field}</dd></div><div className="write-proposal-wide"><dt>Owner-provided value</dt><dd>{proposal.componentFact.value}</dd></div></dl>;
  return <dl className="write-proposal-details"><div><dt>Asset field</dt><dd>{proposal.assetFact.field}</dd></div><div className="write-proposal-wide"><dt>Owner-provided value</dt><dd>{proposal.assetFact.value}</dd></div></dl>;
}
...
export function ErnestChat({ assetId, assetName }: { assetId: string; assetName: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const formData = new FormData(); formData.set("question", question);
    try {
      const result = await askErnest(assetId, emptyState, formData);
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant", text: result.error || result.answer || "I couldn’t answer that right now.", sources: result.sources, proposal: result.proposal }]);
    } catch {
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: "assistant", text: "I couldn’t answer that right now. Please try again." }]);
    } finally { setPending(false); }
  }

  async function saveProposal(messageId: string, proposal: ErnestWriteProposal) {
    if (savingId) return; setSavingId(messageId);
    try {
      const result = await confirmErnestWrite(assetId, proposal);
      setMessages((current) => current.map((message) => message.id === messageId ? { ...message, proposal: result.ok ? undefined : message.proposal, writeResult: result } : message));
    } catch {
      setMessages((current) => current.map((message) => message.id === messageId ? { ...message, writeResult: { ok: false, message: "I couldn’t save that. Please try again." } } : message));
    } finally { setSavingId(null); }
  }

  function dismissProposal(messageId: string) {
    setMessages((current) => current.map((message) => message.id === messageId ? { ...message, proposal: undefined, writeResult: { ok: true, message: "Not saved." } } : message));
  }
    ...
    <section className="ernest-chat" aria-label={`Conversation about ${assetName}`}>
      <div className="ernest-chat-scroll">
        {messages.length === 0 ? (
          <div className="ernest-welcome">
            ...
            <div className="ernest-operate-actions">
              <Link href={`/assets/${assetId}/procedures#checklists`}>✓ <span>Checklist</span></Link>
              <Link className="emergency-action" href={`/assets/${assetId}/procedures#emergency`}>! <span>Emergency</span></Link>
              <button type="button" onClick={() => setInput("I want to record an update: ")}>＋ <span>Update</span></button>
            </div>
            <div className="ernest-prompts">
              <button type="button" onClick={() => setInput("Where can I find ")}>Find something</button>
              <button type="button" onClick={() => setInput("What maintenance should I be thinking about next?")}>Maintenance</button>
              <button type="button" onClick={() => setInput("What do we know about the engine?")}>Ask about equipment</button>
            </div>
          </div>
        ) : (
          <div className="ernest-thread">
            {messages.map((message) => (
              <article className={`chat-message ${message.role}`} key={message.id}>
                ...
                {message.proposal ? <div className="write-proposal"><div className="write-proposal-heading"><span>{message.proposal.kind === "log" ? "Proposed log entry" : "Proposed verified fact"}</span><strong>Nothing is saved until you confirm.</strong></div><p>{message.proposal.summary}</p><ProposalDetails proposal={message.proposal} /><div className="write-proposal-actions"><button className="write-confirm" type="button" disabled={savingId === message.id} onClick={() => saveProposal(message.id, message.proposal!)}>{savingId === message.id ? "Saving…" : "Confirm & save"}</button><button className="write-cancel" type="button" onClick={() => dismissProposal(message.id)}>Don’t save</button></div></div> : null}
      ...
      <div className="ernest-composer-wrap">
        <form className="ernest-composer" onSubmit={submit}>
          <textarea aria-label={`Ask or tell Ernest about ${assetName}`} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder="Ask or tell Ernest anything…" rows={1} maxLength={500} />
          <button type="submit" disabled={pending || !input.trim()} aria-label="Send message">↑</button>
        </form>
        <p className="ernest-composer-note">Ernest can read your trusted knowledge. Changes require confirmation.</p>
      </div>
    </section>
  );
}
